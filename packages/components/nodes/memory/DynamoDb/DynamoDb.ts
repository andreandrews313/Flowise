...
interface RocketStoredMessage extends StoredMessage {
...
class BufferMemoryExtended extends FlowiseMemory implements MemoryMethods {
    ...
    async addNewMessage(
        messages: RocketStoredMessage[],
        client: DynamoDBClient,
        tableName = '',
        dynamoKey: Record<string, AttributeValue> = {},
        messageAttributeName = 'messages'
    ) {
        const timestamp = moment.utc().subtract({ hours: 4 })
        const formattedTimestamp = timestamp.format('yyyy/MM/DD h:mm:ss a')
        const params: UpdateItemCommandInput = {
                    ...
                    L: messages.map((message) => {
                        const dynamoSerializedMessage: DynamoDBSerializedChatMessage = {
                            M: {
                                ...
                                timestamp: {
                                    S: message.timestamp?.toString() ?? formattedTimestamp
                                }
                            }
                        }
            ...
            UpdateExpression:
                'SET #m = list_append(if_not_exists(#m, :empty_list), :m), #ts = :ts, #createdAt = if_not_exists(#createdAt, :createdAt)'
        }
